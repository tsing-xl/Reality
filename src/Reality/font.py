...
# Unused for now
class FontLoader:
    def __init__(self, font_path: str = None, font_name: str | None = None) -> None:
        '''
        Load fonts from given path. Accepts both file path and file-like object.
        ...
        if isinstance(font_path, str):
            try: 
                ...
                self.logout(3, f'Failed to load font from path: {font_path}')
                return
            
            self.logout(0, f'Font loaded from path: {font_path}')

            try:
                font.load(font_name)
            except:
                self.logout(2, f'Failed to load font with name: {font_name}')
                return

            self.logout(0, f'Font loaded with name: {font_name}')

            self._font_name = font_name

        
    def fontName(self) -> str | None:
        '''Get the alternative name of the loaded font.'''
        return self._font_name
    
    def logout(self, lvl: int = 0, text: str = '') -> None:
        consoleLogout(f'{__class__.__name__}@{self}', lvl, text)